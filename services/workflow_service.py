...
class WorkflowService:
            ...
            for row in pendientes:
                ...
                try:               
                    ...
                    if not archivos:
                        ...
                        continue                
                    print(f"Enviando Correo para {escritura}")
    ...
    def ejecutar_envio_certificado(self, payload: dict):
        """Envía un único certificado según información recibida desde el frontend."""
        escritura = payload.get("escritura")
        correo = payload.get("correo")
        if not escritura or not correo:
            raise ValueError("Falta escritura o correo para enviar certificado")

        archivos = []
        if isinstance(payload.get("documentos"), list) and payload.get("documentos"):
            escritura_str = str(escritura)
            carpeta = Path("C:\\descargas\\certificados")
            for doc_name in payload.get("documentos"):
                for p in carpeta.rglob("*"):
                    if p.is_file() and doc_name in p.name and escritura_str in p.name:
                        archivos.append(p)
        if not archivos:
            archivos = self.certificados.buscar_archivos_certificado(escritura)
        ...
        if not archivos:
            self.logs.registrar(
                ...
                proceso="envio_certificado",
                estado="error",
                mensaje="No se encontraron archivos para certificado"
            )
            raise RuntimeError("No se encontraron archivos de certificado para la escritura")
        ...
        self.logs.registrar(
            ...
            proceso="envio_certificado",
            estado="enviado",
            mensaje=f"Certificado enviado para {escritura}"
        )
